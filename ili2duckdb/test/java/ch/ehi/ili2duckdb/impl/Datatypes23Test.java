package ch.ehi.ili2duckdb.impl;
...
public abstract class Datatypes23Test {
    ...
    String [][] expectedValuesAttrNameTable=new String[][] {
    ...
    String [][] expectedValuesTrafoTable=new String[][] {
    ...
    private void doImportIli(boolean withMetadata) throws Exception{
        ...
        try {
            ...
            Ili2dbAssert.assertAttrNameTable(jdbcConnection, expectedValuesAttrNameTable, setup.getSchema());
            Ili2dbAssert.assertTrafoTable(jdbcConnection, expectedValuesTrafoTable, setup.getSchema());
    ...
    public void importXtfAttr() throws Exception{
        ...
        try {
            ...
            String stmtTxt="SELECT * FROM "+setup.prefixName("classattr")+" ORDER BY t_id ASC";
            {
                 ...
                 {
                     Assert.assertTrue(rs.next());
                     Assert.assertEquals("22", rs.getString("aI32id".toLowerCase()));
                     Assert.assertEquals(true, rs.getBoolean("aBoolean".toLowerCase()));
                     Assert.assertEquals("15b6bcce-8772-4595-bf82-f727a665fbf3", rs.getString("aUuid".toLowerCase()));
                     Assert.assertEquals("abc100\"\"''", rs.getString("textLimited".toLowerCase()));
                     Assert.assertEquals("Left", rs.getString("horizAlignment".toLowerCase()));
                     Assert.assertEquals("mailto:ceis@localhost", rs.getString("uritext"));
                     Assert.assertEquals("5", rs.getString("numericInt".toLowerCase()));
                    ...
                    Diff xmlboxDiff = DiffBuilder.compare(Input.fromString(expectedXmlbox)).withTest(Input.fromString(xmlbox))
                     ...
                     Assert.assertEquals("09:00", rs.getString("aTime".toLowerCase()));
                     Assert.assertEquals("abc200\n" + 
                            "end200", rs.getString("mtextLimited".toLowerCase()));
                     Assert.assertEquals("chgAAAAAAAAA0azD", rs.getString("aStandardid".toLowerCase()));
                     Assert.assertEquals("Grunddatensatz.Fixpunkte.LFP.Nummer", rs.getString("aAttribute".toLowerCase()));
                     Assert.assertEquals("2002-09-24", rs.getString("aDate".toLowerCase()));
                     Assert.assertEquals("Top", rs.getString("vertAlignment".toLowerCase()));
                     Assert.assertEquals("ClassA", rs.getString("nametext"));
                     Assert.assertEquals("abc101", rs.getString("textUnlimited".toLowerCase()));
                     Assert.assertEquals("6.0", rs.getString("numericDec".toLowerCase()));
                     Assert.assertEquals("abc201\n" +
                            "end201", rs.getString("mtextUnlimited".toLowerCase()));
                     Assert.assertEquals("1900-01-01 12:30:05.0", rs.getString("aDateTime".toLowerCase()));
                 }
                    {
                        Assert.assertTrue(rs.next());
                         Assert.assertEquals(null, rs.getString("textLimited".toLowerCase()));
                         Assert.assertTrue(rs.wasNull());
                         Assert.assertEquals("textNull", rs.getString("textUnlimited".toLowerCase()));
        ...
        try {
            ...
            String stmtTxt="SELECT * FROM "+setup.prefixName("classattr")+" ORDER BY t_id ASC";
            {
                ...
                ResultSet rs=stmt.getResultSet();
                {
                    Assert.assertTrue(rs.next());
                    Assert.assertEquals("22", rs.getString("aI32id".toLowerCase()));
                    Assert.assertEquals("falschhhhhhh", rs.getString("aBoolean".toLowerCase()));
                    Assert.assertEquals("23A", rs.getString("aUuid".toLowerCase()));
                    Assert.assertEquals("1111111111111111111111111111111111111111", rs.getString("textLimited".toLowerCase()));
                    Assert.assertEquals("42", rs.getString("horizAlignment".toLowerCase()));
                    Assert.assertEquals("mailto:ceis@localhost", rs.getString("uritext"));
                    Assert.assertEquals("im not an int", rs.getString("numericInt".toLowerCase()));
                    Assert.assertEquals("im not a BIG int", rs.getString("numericBigInt".toLowerCase()));
                    String xmlbox = rs.getString("xmlbox");
                    String expectedXmlbox = EXPECTED_XMLBOX;
                    Diff xmlboxDiff = DiffBuilder.compare(Input.fromString(expectedXmlbox)).withTest(Input.fromString(xmlbox))
                            .checkForSimilar().normalizeWhitespace().build();
                    ...
                    Assert.assertEquals("is this a time?", rs.getString("aTime".toLowerCase()));
                    Assert.assertEquals("abc200\n" +
                            "end200", rs.getString("mtextLimited".toLowerCase()));
                    Assert.assertEquals("111", rs.getString("aStandardid".toLowerCase()));
                    Assert.assertEquals("Grunddatensatz.Fixpunkte.LFP.Nummer", rs.getString("aAttribute".toLowerCase()));
                    Assert.assertEquals("one date", rs.getString("aDate".toLowerCase()));
                    Assert.assertEquals("delete", rs.getString("vertAlignment".toLowerCase()));
                    Assert.assertEquals("ClassA", rs.getString("nametext"));
                    Assert.assertEquals("abc101", rs.getString("textUnlimited".toLowerCase()));
                    Assert.assertEquals("im not a decimal", rs.getString("numericDec".toLowerCase()));
                    Assert.assertEquals("abc201\n" +
                            "end201", rs.getString("mtextUnlimited".toLowerCase()));
                    Assert.assertEquals("date and time are nice", rs.getString("aDateTime".toLowerCase()));
                }
                {
                    Assert.assertTrue(rs.next());
                    Assert.assertEquals(null, rs.getString("textLimited".toLowerCase()));
    ...
    protected void assertImportXtfLine_simpleline3(Statement stmt) throws SQLException {
        {
            ...
            while(rs.next()){
                ...
                if(setup.supportsCompoundGeometry()) {
                    ...
                    assertEquals("LINESTRING (2460001 1045001, 2460010 1045010)", geom);
                }
            }
        }
    }
    ...
    protected void assertImportXtfLine_line3(Statement stmt) throws SQLException {
        {
            ...
            while(rs.next()){
                ...
                if(setup.supportsCompoundGeometry()) {
                    ...
                    assertEquals("LINESTRING (2460001 1045001, 2460001.2286122167 1045001.0801162157, 2460001.4546624995 1045001.1671990677, 2460001.6779386075 1045001.2611667924, 2460001.8982309024 1045001.3619311622, 2460002.11533255 1045001.4693975681, 2460002.3290397087 1045001.5834651083, 2460002.539151727 1045001.704026683, 2460002.7454713276 1045001.8309690953, 2460002.9478047937 1045001.9641731572, 2460003.1459621512 1045002.1035138015, 2460003.339757348 1045002.2488601991, 2460003.5290084267 1045002.4000758822, 2460003.7135376967 1045002.5570188722, 2460003.893171901 1045002.7195418131, 2460004.0677423775 1045002.8874921099, 2460004.237085221 1045003.0607120714, 2460004.4010414314 1045003.2390390589, 2460004.5594570693 1045003.4223056388, 2460004.712183395 1045003.6103397394, 2460004.859077012 1045003.8029648125, 2460005 1045004, 2460005.138642302 1045004.2071654596, 2460005.2706842828 1045004.4185989732, 2460005.395994664 1045004.6340903277, 2460005.514448858 1045004.8534252757, 2460005.6259290944 1045005.0763857483, 2460005.730324536 1045005.3027500722, 2460005.8275313913 1045005.5322931898, 2460005.9174530134 1045005.7647868829, 2460006 1045006, 2460010 1045010)", geom);
                }
            }
        }
    }
    ...
    public void exportXtfLine() throws Exception {      
        ...
        try{
            ...
            config.setModels("Datatypes23");
            config.setValidation(false);
            Ili2db.readSettingsFromDb(config); // Model expects 3d coords.
            try{
                Ili2db.run(config,null);
            }catch(Exception ex){
                EhiLogger.logError(ex);
            ...
            IoxEvent event=null;
              ...
              }
             {
                ...
                if(setup.supportsCompoundGeometry()) {
                    ...
                    Assert.assertEquals("POLYLINE {sequence SEGMENTS {segment [COORD {C1 2460001.000, C2 1045001.000}, COORD {C1 2460001.229, C2 1045001.080}, COORD {C1 2460001.455, C2 1045001.167}, COORD {C1 2460001.678, C2 1045001.261}, COORD {C1 2460001.898, C2 1045001.362}, COORD {C1 2460002.115, C2 1045001.469}, COORD {C1 2460002.329, C2 1045001.583}, COORD {C1 2460002.539, C2 1045001.704}, COORD {C1 2460002.745, C2 1045001.831}, COORD {C1 2460002.948, C2 1045001.964}, COORD {C1 2460003.146, C2 1045002.104}, COORD {C1 2460003.340, C2 1045002.249}, COORD {C1 2460003.529, C2 1045002.400}, COORD {C1 2460003.714, C2 1045002.557}, COORD {C1 2460003.893, C2 1045002.720}, COORD {C1 2460004.068, C2 1045002.887}, COORD {C1 2460004.237, C2 1045003.061}, COORD {C1 2460004.401, C2 1045003.239}, COORD {C1 2460004.559, C2 1045003.422}, COORD {C1 2460004.712, C2 1045003.610}, COORD {C1 2460004.859, C2 1045003.803}, COORD {C1 2460005.000, C2 1045004.000}, COORD {C1 2460005.139, C2 1045004.207}, COORD {C1 2460005.271, C2 1045004.419}, COORD {C1 2460005.396, C2 1045004.634}, COORD {C1 2460005.514, C2 1045004.853}, COORD {C1 2460005.626, C2 1045005.076}, COORD {C1 2460005.730, C2 1045005.303}, COORD {C1 2460005.828, C2 1045005.532}, COORD {C1 2460005.917, C2 1045005.765}, COORD {C1 2460006.000, C2 1045006.000}, COORD {C1 2460010.000, C2 1045010.000}]}}", lineSegment.toString());
                }
             }
        }finally{
        }
