 ...
 */
public class PgSequenceBasedPooledLoIdGen implements DbIdGen {
	...
	public void initDbDefs(ch.ehi.sqlgen.generator.Generator gen) {
        ...
        if(BLOCK_SIZE>1){
            stmt=stmt+" INCREMENT "+BLOCK_SIZE;
        }
		stmt=stmt+";";
	...
	long idBlockStart=0;
	long lastLocalId=0;
	private	final long BLOCK_SIZE=20;
	@Override
	public long newObjSqlId(){
		long ret;
		if(idBlockStart!=0 && lastLocalId+1<BLOCK_SIZE){
			lastLocalId++;
			ret=idBlockStart+lastLocalId;
		}else{
			lastLocalId=0;
			idBlockStart=getSeqCount();
            ret=idBlockStart+lastLocalId;
		}
		return ret;
	}
