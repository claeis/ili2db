...
-- MultiSurface2.FlaecheStruktur2D
CREATE TABLE MultiSurface.flaechestruktur2d (
  T_Id bigint PRIMARY KEY DEFAULT nextval('MultiSurface.t_ili2db_seq')
  ,T_basket bigint NOT NULL
  ,T_Seq bigint NULL
  ,multiflaeche2d_flaechen bigint NULL
)
;
SELECT AddGeometryColumn('multisurface','flaechestruktur2d','flaeche',(SELECT srid FROM SPATIAL_REF_SYS WHERE AUTH_NAME='EPSG' AND AUTH_SRID=21781),'CURVEPOLYGON',2);
COMMENT ON TABLE MultiSurface.flaechestruktur2d IS '@iliname MultiSurface2.FlaecheStruktur2D';
COMMENT ON COLUMN MultiSurface.flaechestruktur2d.flaeche IS '@iliname Flaeche';
COMMENT ON COLUMN MultiSurface.flaechestruktur2d.multiflaeche2d_flaechen IS '@iliname MultiSurface2.MultiFlaeche2D.Flaechen';
-- MultiSurface2.MultiFlaeche2D
CREATE TABLE MultiSurface.multiflaeche2d (
  T_Id bigint PRIMARY KEY DEFAULT nextval('MultiSurface.t_ili2db_seq')
  ,T_basket bigint NOT NULL
  ,T_Seq bigint NULL
)
;
COMMENT ON TABLE MultiSurface.multiflaeche2d IS '@iliname MultiSurface2.MultiFlaeche2D';
-- MultiSurface2.TestA.ClassA1
...
;
SELECT AddGeometryColumn('multisurface','classa1','geom',(SELECT srid FROM SPATIAL_REF_SYS WHERE AUTH_NAME='EPSG' AND AUTH_SRID=21781),'MULTISURFACE',2);
COMMENT ON TABLE MultiSurface.classa1 IS '@iliname MultiSurface2.TestA.ClassA1';
...
;
ALTER TABLE MultiSurface.flaechestruktur2d ADD CONSTRAINT flaechestruktur2d_T_basket_fkey FOREIGN KEY ( T_basket ) REFERENCES MultiSurface.T_ILI2DB_BASKET DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE MultiSurface.flaechestruktur2d ADD CONSTRAINT flaechestruktur2d_multiflaeche2d_flaechen_fkey FOREIGN KEY ( multiflaeche2d_flaechen ) REFERENCES MultiSurface.multiflaeche2d DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE MultiSurface.multiflaeche2d ADD CONSTRAINT multiflaeche2d_T_basket_fkey FOREIGN KEY ( T_basket ) REFERENCES MultiSurface.T_ILI2DB_BASKET DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE MultiSurface.T_ILI2DB_MODEL ADD CONSTRAINT T_ILI2DB_MODEL_modelName_iliversion_key UNIQUE (modelName,iliversion)
;
